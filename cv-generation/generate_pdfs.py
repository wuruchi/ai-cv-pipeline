...
def get_all_available_jsons(json_dir: str) -> list[str]:
    logging.info(f"Fetching all available JSON CVs from {json_dir}...")
    return [
...
def render_cv_html(cv: CV) -> str:
    ...
    cv_data["avatar_url"] = AvatarProvider.get_url(f"{cv.avatar_seed}")
...
def generate_pdfs_from_available_jsons(json_dir: str = "./output"):
    logging.info("Generating PDF CVs from available JSONs...")
    json_files = get_all_available_jsons(json_dir)
